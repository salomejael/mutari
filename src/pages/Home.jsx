...
export default function Home() {
    ...
    if (myProfile?.latitude && myProfile?.longitude) {
      filtered = filtered
        .filter(item => {
          ...
          const dist = haversine(myProfile.latitude, myProfile.longitude, item.profiles.latitude, item.profiles.longitude)
          return dist <= (myProfile.search_radius_km || 50)
        })
        .map(item => {
  ...
  const nextItem = () => {
    setImageIndex(0)
    if (current + 1 < filteredItems.length) {
  ...
  const EmptyState = ({ hasFilters }) => (
    <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px', textAlign: 'center', width: '280px' }}>
      ...
      {hasFilters ? (
        <p style={{ fontSize: '14px', fontFamily: 'Inter, system-ui, sans-serif', color: 'var(--text-secondary)' }}>No items match your filters</p>
      ) : (
        <>
          <p style={{ fontSize: '14px', fontFamily: 'Inter, system-ui, sans-serif', fontWeight: '600', color: 'var(--text-secondary)' }}>No items found nearby</p>
          <p style={{ fontSize: '13px', fontFamily: 'Inter, system-ui, sans-serif', lineHeight: '1.5', color: 'var(--text-secondary)' }}>Try adjusting your filter settings or update your location and search radius to discover more items.</p>
          <button onClick={() => navigate('/settings')} className="edit-settings-btn" style={{ marginTop: '8px' }}>
            Edit Location
          </button>
        </>
      )}
  ...
  if (!hasOwnItems) return (
    <div style={{ minHeight: '100vh', backgroundColor: 'var(--bg)', fontFamily: 'Inter, system-ui, sans-serif', paddingTop: '80px' }}>
      <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px', textAlign: 'center', width: '300px' }}>
        ...
        <button onClick={() => navigate("/profile?addItem=true")} className="edit-settings-btn" style={{ marginTop: '8px' }}>
          Add your first item
        </button>
      </div>
    </div>
  ...
  return (
    ...
    >
      ...
      <div style={{ margin: '12px 24px', backgroundColor: 'var(--bg-card)', borderRadius: '28px', overflow: 'hidden', boxShadow: '0 8px 40px rgba(0,0,0,0.08)' }}>
        ...
        <div style={{ padding: '20px 24px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
            ...
            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
              {item.size && (
                <span style={{ fontSize: '13px', fontWeight: '600', color: 'var(--text-secondary)', backgroundColor: 'var(--bg-input)', padding: '4px 12px', borderRadius: '8px' }}>{item.size}</span>
              )}
              {item.condition && (
                <span style={{ fontSize: '13px', fontWeight: '600', color: 'var(--text-secondary)', backgroundColor: 'var(--bg-input)', padding: '4px 12px', borderRadius: '8px' }}>{item.condition}</span>
              )}
            </div>
          </div>
          {item.description && (
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: '24px', padding: '8px 24px' }}>
        <button onClick={handleDislike} style={{ width: '64px', height: '64px', borderRadius: '50%', border: '2px solid var(--accent)', backgroundColor: '#FFFFFF', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 16px rgba(185,28,28,0.2)' }}>
        ...
        <button onClick={handleLike} style={{ width: '64px', height: '64px', borderRadius: '50%', border: 'none', backgroundColor: 'var(--accent)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 16px rgba(185,28,28,0.3)' }}>
