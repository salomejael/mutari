  ...
  if (activeConv) {
    ...
    return (
      <div style={{ minHeight: '100vh', backgroundColor: 'var(--bg)', fontFamily: 'Inter, system-ui, sans-serif', display: 'flex', flexDirection: 'column' }}>
        <div style={{ position: 'fixed', top: '0', left: '0', right: '0', backgroundColor: '#ffd6ff', padding: '16px 24px', display: 'grid', gridTemplateColumns: '1fr auto 1fr', alignItems: 'center', zIndex: '100', height: '64px', boxSizing: 'border-box' }}>
      ...
      {conversations.length === 0 ? (
        ...
        <div style={{ padding: '16px 24px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {conversations.map(conv => {
            ...
            return (
              <div key={conv.id} onClick={() => setActiveConv(conv)} style={{ backgroundColor: 'var(--bg-card)', borderRadius: '20px', padding: '16px 20px', display: 'flex', alignItems: 'center', gap: '14px', cursor: 'pointer', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>Tap to open conversation</p>
                </div>
